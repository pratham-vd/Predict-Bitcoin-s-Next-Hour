...
# Single GBM path simulator. Used as fallback for n_steps > 1.
def simulate_cyber_gbm(S0, mu, sigma_series, H, M, params, bar_sigma2,
    ...
    for t in range(1, n_steps + 1):
        ...
        if idx < len(info_filter):
            sigma2 *= 1 + 0.1 * info_filter.iloc[idx]
...
# Monte Carlo simulation. Vectorised for n_steps=1 (the common case).
def simulate_mc(S0, mu, sigma_series, H, M, bar_sigma2, nu,
    ...
    if n_steps == 1:
        ...
        sigma2 *= 1.0 + 0.1 * float(info_filter.iloc[-1])
        sigma2 = max(eps, min(float(sigma2), 0.5))

        # Student-t normalised to variance=1, all shocks generated in one call
        Z = np.random.standard_t(nu, size=n_sims) * np.sqrt((nu - 2.0) / nu)
        S_next = S0 * np.exp((mu - 0.5 * sigma2) + np.sqrt(sigma2) * Z)
    ...
    # Fallback loop for multi-step forecasting
...
# 30-day backtest. At each bar i, predicts bar i+1 using only data up to bar i.
def backtest_btc_hourly(prices, train_window=168, test_window=None, n_sims=10_000):
    ...
    for i in tqdm(range(train_window, train_window + test_window)):
        train_ret = log_ret.iloc[i - train_window:i]
        train_prices = prices.iloc[i - train_window:i + 1]
        ...
        try:
            redundancy = 1 + 0.05 * np.log1p(train_prices.rolling(5).var() / train_prices.rolling(20).var())
...
# Compute coverage, average width, and mean Winkler score.
...
if __name__ == '__main__':
    ...
    backtest_df = backtest_btc_hourly(prices, train_window=168, n_sims=10_000)
